...
(function($) {
    $.fn.amazingaudioplayer = function(options) {
        var PlayerSkin = function(amazingPlayer, container, options, id) {
            ...
            var $play = $("<div class='amazingaudioplayer-play amazingaudioplayer-icon'><i class='am-icon-play'></i></div>");
            var $pause = $("<div class='amazingaudioplayer-pause amazingaudioplayer-icon display-none'><i class='am-icon-pause'></i></div>");
            ...
            if (this.options.showprevnext) {
                var $prev = $("<div class='amazingaudioplayer-prev amazingaudioplayer-icon'><i class='am-icon-fast-backward'></i></div>");
                var $next = $("<div class='amazingaudioplayer-next amazingaudioplayer-icon'><i class='am-icon-fast-forward'></i></div>");
                $prev.appendTo($bar);
                $next.appendTo($bar);
                $prev.click(function() {
            ...
            if (this.options.showloop) {
                var $loop = $("<div class='amazingaudioplayer-loop amazingaudioplayer-icon'><i class='am-icon-reorder'></i></div>");
                $loop.appendTo($bar);
                $loop.click(function() {
            ...
            if (this.options.showvolume && !AmazingAudioPlatforms.isIOS() && !AmazingAudioPlatforms.isAndroid()) {
                ...
                this.$volumeButton = $("<div class='amazingaudioplayer-volume-button amazingaudioplayer-icon'><i class='am-icon-volume-up'></i></div>");
                this.$volumeButton.appendTo(this.$volume);
                this.$volumeButton.click(function() {
                    var volume = parseFloat(instance.amazingPlayer.audioPlayer.getVolume()).toFixed(1);
                    if (volume > 0) {
                ...
                if (this.options.showvolumebar) {
                    ...
                    this.$volumeBarAdjust.bind(eStart, function(e) {
                        ...
                        var vol = parseFloat(1 - (e0.pageY - instance.$volumeBarAdjust.offset().top) / instance.$volumeBarAdjust.height()).toFixed(1);
            ...
            if (this.options.showtime) {
                ...
                this.container.bind("amazingaudioplayer.playprogress", function(event, data) {
                    ...
                    if (data.live) {
                        ...
                        t = instance.options.timeformat.replace("%CURRENT%", current).replace("%DURATION%", duration).replace("%LEFT%", left);
                    }
                    $time.html(t);
                });
                this.container.bind("amazingaudioplayer.played", function(event, currentItem) {
            ...
            if (this.options.showtracklist) {
                ...
                for (var i = 0; i < this.amazingPlayer.elemLength; i++) {
                    ...
                    $track.hover(function() {
                        ...
                        if ($(this).data("order") !== instance.amazingPlayer.currentItem) {
        ...
        FlashHTML5Player.prototype = {
            ...
            initHtml5: function() {
                var html5Object = $("<audio class='display-none' preload='" + (this.amazingPlayer.options.preloadaudio ? "auto" : "none") + "'></audio>");
        ...
        AmazingAudioPlayer.prototype = {
            ...
            readTags: function() {
                var instance = this;
                $(".amazingaudioplayer-audios", this.container).find("a").each(function() {
                    var $this = $(this);
                    var audioSource = [];
            ...
            init: function(instance) {
                ...
                var firstId = 0;
                if ("firstaudioid" in params && !isNaN(params.firstaudioid) && params.firstaudioid >= 0 && params.firstaudioid < instance.elemLength) {
                ...
                if (instance.options.defaultvolume >= 0) {
                    instance.setVolume(parseFloat(instance.options.defaultvolume / 100).toFixed(1));
                }
            ...
            audioRun: function(index, autoPlay) {
                ...
                var nextItem;
                if (index === -2) {
                    nextItem = this.currentItem <= 0 ? this.elemLength - 1 : this.currentItem - 1;
                } else if (index === -1) {
                    nextItem = this.currentItem >= this.elemLength - 1 ? 0 : this.currentItem + 1;
                } else {
                    nextItem = index;
                }
            ...
            onAudioEnded: function() {
                ...
                switch (this.options.loop) {
                    case 0:
                        if (!this.options.noncontinous && this.currentItem < this.elemLength - 1) {
        ...
        this.each(function() {
            ...
            if ($(this).data("skin") && typeof AMAZINGAUDIOPLAYER_SKIN_OPTIONS !== "undefined") {
                if ($(this).data("skin") in AMAZINGAUDIOPLAYER_SKIN_OPTIONS) {
                    this.options = $.extend({}, AMAZINGAUDIOPLAYER_SKIN_OPTIONS[$(this).data("skin")], this.options);
                }
            }
            var instance = this;
            ...
            if ($("div#amazingaudioplayer_options").length) {
                $.each($("div#amazingaudioplayer_options").data(), function(key, value) {
                    instance.options[key.toLowerCase()] = value;
                });
            }
            var searchoptions = {};
    ...
    function showExpressInstall(att, par, replaceElemIdStr, callbackFn) {
        ...
        if (obj) {
            ...
            if (typeof par.flashvars != UNDEF) par.flashvars +=
                "&" + fv;
            else par.flashvars = fv; if (ua.ie && ua.win && obj.readyState != 4) {
            ...
            if (ua.w3 && !(ua.wk && ua.wk < 312) && swfUrlStr && replaceElemIdStr && widthStr && heightStr && swfVersionStr) {
                ...
                addDomLoadEvent(function() {
                    ...
                    if (hasPlayerVersion(swfVersionStr)) {
                        ...
                        return
                    } else setVisibility(replaceElemIdStr, true); if (callbackFn) callbackFn(callbackObj)
                })
...
$(function() {
    ...
    $('#form-tabs li').on('click', function() {
        ...
        $('#music_input').data('filter', filter).attr({'placeholder': holder[filter], 'pattern': holder['pattern-'+filter]}).removeClass('am-field-valid am-field-error am-active').closest('.am-form-group').removeClass('am-form-success am-form-error').find('.am-alert').hide();
        if (filter === 'url') {
            $('.music-type').hide();
        } else {
            $('.music-type').show();
    ...
    $('#form-vld').validator({
        ...
        onInValid: function(validity) {
            ...
            var msg = msgs[$field.data('filter')] || this.getValidationMessage(validity);
            if (!$alert.length) {
                $alert = $('<div class="am-alert am-alert-danger am-animation-shake"></div>').hide().
                appendTo($group);
            }
            $alert.html(msg).show();
        },
                ...
                return $.ajax({
                    ...
                    success: function(result){
                        if (result.status === '200') {
                            ...
                            var html = '<div id="player" class="audio-player"><div class="amazingaudioplayer-audios">';
